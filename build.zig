...
fn buildProgram(b: *Build, options: ExampleOptions) !void {
    const main_src = "src/main.zig";
    const name = "todo-prog";
    ...
    if (!options.target.result.isWasm()) {
        ...
        example.addCSourceFile(.{ .file = b.path("src/stb/stb_truetype.c"), .flags = &.{"-g"} }); // -g adds debug info, makes it easier to debug
    ...
    b.step("run", "Run program").dependOn(&run.?.step);
}
...
fn buildShaders(b: *Build, target: Build.ResolvedTarget) void {
    ...
    const shdc_step = b.step("shaders", "Compile shaders (needs tools/sokol-tools-bin)");
